...
const isSimultaneousFlash = () => {
  //sums the matrix by flattening it to one array, then adds all elements together
  return (
    inputs.flat().reduce((total, num) => {
      return total + num;
    }) == 0
  );
};
...
reader1.on("close", () => {
  ...
  for (let j = 0; j < 10; j++) {
    console.log(`inputs at end ${days}: ${j} |  ${inputs[j]} `);
  }
  console.log("--------------------------------------------------");
